    ...
    if var is not None:
        ...
        if var_content in true_strings:
            ...
            raise UserInputError(
                ...
                'Current value is %s.' % (
                    var_name, ', '.join(
                        true_strings), ', '.join(false_strings),
                    var))
...
def set_tf_cuda_version(environ_cp):
    ...
    for _ in range(_DEFAULT_PROMPT_ASK_ATTEMPTS):
        ...
        cuda_toolkit_path_full = os.path.join(
            cuda_toolkit_path, cuda_rt_lib_path)
...
def main():
    ...
    if environ_cp.get('TF_NEED_OPENCL_SYCL') == '1':
        ...
        set_action_env_var(environ_cp, 'TF_NEED_COMPUTECPP',
                           'ComputeCPP', True)
    ...
    config_info_line(
        'monolithic', 'Config for mostly static monolithic build.')
