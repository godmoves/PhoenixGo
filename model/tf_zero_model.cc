...
TfZeroModel::TfZeroModel(int gpu) : m_session(nullptr), m_gpu(gpu) {}

TfZeroModel::~TfZeroModel() {
...
int TfZeroModel::Init(const ModelConfig &model_config) {
  ...
  if (model_config.enable_xla()) {
    options.config.mutable_graph_options()
        ...
        ->set_global_jit_level(tf::OptimizerOptions::ON_1);
  }
...
int TfZeroModel::Forward(const std::vector<std::vector<bool>> &inputs,
                       std::vector<std::vector<float>> &policy,
                       std::vector<float> &value) {
...
int TfZeroModel::GetGlobalStep(int &global_step) {
  std::vector<tf::Tensor> network_outputs;
...
void TfZeroModel::SetMKLEnv(const ModelConfig &model_config) {
