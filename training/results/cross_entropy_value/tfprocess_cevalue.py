...
class TFProcess_CEValue:
    ...
    def init_net(self, planes, probs, winner, gpus_num):
            ...
            for i in range(gpus_num):
                with tf.device("/gpu:%d" % i):
                    with tf.name_scope("tower_%d" % i):
                        loss, policy_loss, value_loss, reg_term, y_conv = self.tower_loss(
                        ...
                        tower_value_loss.append(value_loss)
        ...
        self.value_loss = tf.reduce_mean(tower_value_loss)
    ...
    def tower_loss(self, x, y_, z_):
        ...
        # TODO: figure out whether we should use softmax CE
        v_labels = tf.concat([(z_ + 1.0) / 2.0, (z_ - 1.0) / -2.0], 1)
        v_logits = tf.concat([(z_conv + 1.0) / 2.0, (z_conv - 1.0) / -2.0], 1)
        ...
        # Loss on value head
        value_loss = \
            tf.nn.softmax_cross_entropy_with_logits_v2(labels=v_labels,
                                                       logits=v_logits)
        ...
        # want to reduce the factor in front of self.value_loss here.
        loss = 1.0 * policy_loss + 1.0 * value_loss + reg_term

        return loss, policy_loss, value_loss, reg_term, y_conv
    ...
    def measure_loss(self, batch, training=False):
        ...
        ops = [self.policy_loss, self.value_loss, self.reg_term, self.accuracy]
        if training:
        ...
        r = self.session.run(ops, feed_dict={self.training: training,
                                             ...
                                             self.probs: batch[1],
                                             self.winner: batch[2]})
        return {'policy': r[0], 'value': r[1], 'reg': r[2],
    ...
    def process(self, train_data, test_data):
        ...
        while True:
            ...
            if steps % info_steps == 0:
                ...
                self.logger.info("step {} lr={:g} policy={:g} value={:g} reg={:g} total={:g} ({:g} pos/s)".format(
                    steps, learning_rate, stats.mean('policy'), stats.mean('value'), stats.mean('reg'),
            ...
            if steps % 8000 == 0:
                ...
                self.logger.info("step {}, policy={:g} training accuracy={:g}%, value={:g}".format(
                    steps, test_stats.mean('policy'),
                    test_stats.mean('accuracy') * 100.0,
                    test_stats.mean('value')))
