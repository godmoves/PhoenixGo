...
def conv_bn_wider(weights, next_weights, inputs, channels,
    ...
    w_conv_new = np.array(weights[0]).reshape(
        channels, inputs, 3, 3)[rand, :, :, :]
    ...
    if noise_std == 0 and verify:
        ...
        old1 = convolve(np.array(weights[0]).reshape(
            channels, inputs, 3, 3), x, bn=[weights[2], weights[3]])
        old2 = convolve(
            np.array(next_weights[0]).reshape(-1, channels, w_filter, w_filter), old1)
        new1 = convolve(np.array(w_conv_new).reshape(
            channels + new_channels, inputs, 3, 3), x, bn=[w_bn_means, w_bn_vars])
        new2 = convolve(np.array(
            next_weights_new[0]).reshape(-1, channels + new_channels, w_filter, w_filter), new1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Add filters/blocks to existing network such that the output is preserved.')
    ...
    parser.add_argument("--dir_alpha", nargs='?', help="""Dirichlet distribution parameter for input weight distribution for replicated channels. """
    ...
    if new_blocks > 0:
        ...
        new_block = [new_block_conv, new_block_bias,
                     new_block_bn_mean, new_block_bn_variances]
    ...
    if args.add_inputs > 0:
        ...
        noise = np.random.normal(
            0, noise_std, [channels, args.add_inputs, 3, 3])
